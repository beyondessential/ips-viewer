...
const StyledTooltip = styled((props) => (
  ...
  & .MuiTooltip-tooltip {
    background-color: ${COLORS.FONT_DARKGREY};
    color: ${COLORS.WHITE};
    font-weight: 400;
    font-size: 11px;
    line-height: 15px;
...
export const OuterLabelFieldWrapper = React.memo(
  React.forwardRef(
    (
      { children, required, label, infoTooltip, style, className, size },
      ref
    ) => (
      <div style={style} className={className} ref={ref}>
        ...
        {infoTooltip && (
          <StyledTooltip arrow placement="top" title={infoTooltip}>
            <Icon className="fa fa-info-circle" color={COLORS.FONT_MIDGREY} />
          </StyledTooltip>
        )}
        {children}
      </div>
